...
void main() {
  ...
  test('sleep migration creates expected tables', () async {
    ...
    expect(rows.map((r) => r.read<String>('name')).toList(growable: false), [
      'sleep_canonical_heart_rate_samples',
      'sleep_canonical_sessions',
      'sleep_canonical_stage_segments',
      'sleep_nightly_analyses',
  ...
  test(
    'canonical daos insert/query and delete-by-range semantics work',
    () async {
      ...
      final rawAfterDelete = await rawDao.findBySourceHash(
        'hash-raw-for-session-1',
      );
  ...
  test(
    'derived analyses dao insert/query/delete by night range works',
    () async {
